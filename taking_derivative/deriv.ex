defmodule Derivative do
    ...
    def deriv({:var,x},_) do 
        {:var,x} 
    end

    # 4. derivation general power rule 
    def deriv({:pow, e1, {:num, n}}, e1) do 
        {:mul, {:num, n}, {:pow, e1, {:num, n-1}}} 
    end

    # 5. derivattion of squar root
    def deriv({:sqrt, e1}, x) do 
        {:frac, deriv(e1,x), {:mul, {:num, 2}, {:sqrt, e1}}} 
    end
    ...
    def deriv({:sin, e1}, x) do 
        {:mul, deriv(e1,x), {:cos, e1}} 
    end

    def deriv({:cos, e1}, x) do 
        {:mul, {:num, -1}, {:mul, {deriv(e1,x), {:sin, e1}}}} 
    end 
    ...
    def simp_add(e1, {:num, 0}) do 
        e1 
    end

    def simp_add({:num, 0}, e1) do 
        e1 
    end  

    def simp_add({:num, n1}, {:num, n2}) do 
        {:num, n1+n2} 
    end

    def simp_add({:var, v}, {:var, v}) do 
        {:mul, {:num, 2}, {:var, v}} 
    end
    ...
    def simp_mul(_, {:num,0}) do 
        {:num, 0} 
    end

    def simp_mul({:num, 0}, _) do 
        {:num, 0} 
    end

    def simp_mul(e1, {:num,1}) do
        e1 
    end

    def simp_mul({:num, 1}, e1) do 
        e1 
    end

    def simp_mul({:num, n1}, {:mul, {:num, n2}, e2}) do
        {:mul, {:num, n1*n2}, e2}
    end

    def simp_mul({:num, n1}, {:mul, e2, {:num, n2}}) do
        {:mul, {:num, n1*n2}, e2}
    end

    def simp_mul({:mul, {:num, n1}, e1}, {:num, n2}) do
        {:mul, {:num, n1*n2}, e1}
    end

    def simp_mul({:mul, e1, {:num, n1}}, {:num, n2}) do
        {:mul, {:num, n1*n2}, e1}
    end

    def simp_mul({:num, n}, {:num, m}) do 
        {:num, n*m} 
    end

    def simp_mul({:var, v}, {:var, v}) do 
        {:pow, {:var, v}, {:num, 2}} 
    end

    def simp_mul({:var, v}, {:pow, {:var, v}, {:num, n}}) do 
        {:pow, {:var, v}, {:num, n+1}} 
    end  

    def simp_mul({:pow, {:var, v}, {:num, n}}, {:var, v}) do 
        {:pow, {:var, v}, {:num, n+1}} 
    end

    #exponential
    def simp_pow(_,{:num, 0}) do
        1 
    end  

    def simp_pow(e1,{:num, 1}) do  
        e1 
    end

    def simplify_pow(e1,e2) do
        {:pow, e1, e2}
    end
...
Derivative.test()
IO.puts("You came a long way. Please be proud of yourself")
